...
function CalcTotalBoards(longWallWidthInFeet:number, wideWallWidthInFeet:number)
{
   ...
   //meaning he needs to only make two additional cuts.
...
export default function BuildHouse(longWallWidthInFeet:number, wideWallWidthInFeet:number)
{
    ...
    console.log("Total posts to purchase after accounting for waste: " + (Number(totalPostsPlusWaste) + 4));  //Account for four posts in every house.
}